...
Array.prototype.copy = function () {
  ...
  for (var i = 0; i < this.length; i++) {
    if (isPushProgram(this[i])) newCopy.push(this[i].copy());
    else newCopy.push(this[i]);
  }
...
function pushInstructionDo(inInterpreter, inStack) {
  if (inInterpreter.codeStack.length > 0) {
    ...
    inInterpreter.execStack.push(
      inInterpreter.codeStack[inInterpreter.codeStack.length - 1]
    );
  }
}
...
function pushInstructionIf(inInterpreter, inStack) {
  if (inStack.length > 1 && inInterpreter.boolStack.length > 0) {
    ...
    if (cond) inInterpreter.execStack.push(ifTrue);
    else inInterpreter.execStack.push(ifFalse);
  }
}
...
function pushInstructionDivide(inInterpreter, inStack) {
  if (inStack.length > 1) {
    ...
    if (o1 == 0.0) {
      // oops!  protect the divide and return;
      inStack.push(o1);
      return;
    }
...
function pushInstructionDefine(inInterpreter, inStack) {
  if (inStack.length > 0 && inInterpreter.nameStack.length > 0) {
    inInterpreter[inInterpreter.nameStack.pop()] = new pushDefine(
      inStack,
      inStack.pop()
    );
  }
...
function pushInterpreter() {
  ...
  this["INTEGER.FROMFLOAT"] = new pushInstruction(
    this.intStack,
    pushInstructionFromFloat
  );
  this["BOOLEAN.FROMFLOAT"] = new pushInstruction(
    this.boolStack,
    pushInstructionFromFloat
  );

  this["FLOAT.FROMINTEGER"] = new pushInstruction(
    this.floatStack,
    pushInstructionFromInteger
  );
  this["BOOLEAN.FROMINTEGER"] = new pushInstruction(
    this.boolStack,
    pushInstructionFromInteger
  );

  this["FLOAT.FROMBOOLEAN"] = new pushInstruction(
    this.floatStack,
    pushInstructionFromBoolean
  );
  this["INTEGER.FROMBOOLEAN"] = new pushInstruction(
    this.integerStack,
    pushInstructionFromBoolean
  );
  ...
  this["INTEGER.>"] = new pushInstruction(
    this.intStack,
    pushInstructionGreaterThan
  );
  this["FLOAT.>"] = new pushInstruction(
    this.floatStack,
    pushInstructionGreaterThan
  );
  ...
  this["BOOLEAN.="] = new pushInstruction(
    this.boolStack,
    pushInstructionEquals
  );
  ...
  this["FLOAT.DEFINE"] = new pushInstruction(
    this.floatStack,
    pushInstructionDefine
  );
  this["INTEGER.DEFINE"] = new pushInstruction(
    this.intStack,
    pushInstructionDefine
  );
  this["CODE.DEFINE"] = new pushInstruction(
    this.codeStack,
    pushInstructionDefine
  );
  this["EXEC.DEFINE"] = new pushInstruction(
    this.execStack,
    pushInstructionDefine
  );
  this["BOOLEAN.DEFINE"] = new pushInstruction(
    this.boolStack,
    pushInstructionDefine
  );
  ...
  this["FLOAT.YANK"] = new pushInstruction(
    this.floatStack,
    pushInstructionYank
  );
  this["INTEGER.YANK"] = new pushInstruction(
    this.intStack,
    pushInstructionYank
  );
  ...
  this["BOOLEAN.YANK"] = new pushInstruction(
    this.boolStack,
    pushInstructionYank
  );
  this["NAME.YANK"] = new pushInstruction(this.nameStack, pushInstructionYank);

  this["FLOAT.YANKDUP"] = new pushInstruction(
    this.floatStack,
    pushInstructionYankDup
  );
  this["INTEGER.YANKDUP"] = new pushInstruction(
    this.intStack,
    pushInstructionYankDup
  );
  this["CODE.YANKDUP"] = new pushInstruction(
    this.codeStack,
    pushInstructionYankDup
  );
  this["EXEC.YANKDUP"] = new pushInstruction(
    this.execStack,
    pushInstructionYankDup
  );
  this["BOOLEAN.YANKDUP"] = new pushInstruction(
    this.boolStack,
    pushInstructionYankDup
  );
  this["NAME.YANKDUP"] = new pushInstruction(
    this.nameStack,
    pushInstructionYankDup
  );

  this["FLOAT.SHOVE"] = new pushInstruction(
    this.floatStack,
    pushInstructionShove
  );
  this["INTEGER.SHOVE"] = new pushInstruction(
    this.intStack,
    pushInstructionShove
  );
  this["CODE.SHOVE"] = new pushInstruction(
    this.codeStack,
    pushInstructionShove
  );
  this["EXEC.SHOVE"] = new pushInstruction(
    this.execStack,
    pushInstructionShove
  );
  this["BOOLEAN.SHOVE"] = new pushInstruction(
    this.boolStack,
    pushInstructionShove
  );
  this["NAME.SHOVE"] = new pushInstruction(
    this.nameStack,
    pushInstructionShove
  );
  ...
  this["FLOAT.STACKDEPTH"] = new pushInstruction(
    this.floatStack,
    pushInstructionStackdepth
  );
  this["INTEGER.STACKDEPTH"] = new pushInstruction(
    this.intStack,
    pushInstructionStackdepth
  );
  this["CODE.STACKDEPTH"] = new pushInstruction(
    this.codeStack,
    pushInstructionStackdepth
  );
  this["EXEC.STACKDEPTH"] = new pushInstruction(
    this.execStack,
    pushInstructionStackdepth
  );
  this["BOOLEAN.STACKDEPTH"] = new pushInstruction(
    this.boolStack,
    pushInstructionStackdepth
  );
  this["NAME.STACKDEPTH"] = new pushInstruction(
    this.nameStack,
    pushInstructionStackdepth
  );

  this["FLOAT.SWAP"] = new pushInstruction(
    this.floatStack,
    pushInstructionSwap
  );
  this["INTEGER.SWAP"] = new pushInstruction(
    this.intStack,
    pushInstructionSwap
  );
  ...
  this["BOOLEAN.SWAP"] = new pushInstruction(
    this.boolStack,
    pushInstructionSwap
  );
  this["NAME.SWAP"] = new pushInstruction(this.nameStack, pushInstructionSwap);

  this["FLOAT.FLUSH"] = new pushInstruction(
    this.floatStack,
    pushInstructionFlush
  );
  this["INTEGER.FLUSH"] = new pushInstruction(
    this.intStack,
    pushInstructionFlush
  );
  this["CODE.FLUSH"] = new pushInstruction(
    this.codeStack,
    pushInstructionFlush
  );
  this["EXEC.FLUSH"] = new pushInstruction(
    this.execStack,
    pushInstructionFlush
  );
  this["BOOLEAN.FLUSH"] = new pushInstruction(
    this.boolStack,
    pushInstructionFlush
  );
  this["NAME.FLUSH"] = new pushInstruction(
    this.nameStack,
    pushInstructionFlush
  );
  ...
  this["CODE.QUOTE"] = new pushInstruction(
    this.codeStack,
    pushInstructionQuote
  );
  ...
  this["CODE.CONTAINS"] = new pushInstruction(
    this.codeStack,
    pushInstructionContains
  );
  ...
  this["CODE.DO*RANGE"] = new pushInstruction(
    this.codeStack,
    pushInstructionDoRange
  );
  this["CODE.DO*COUNT"] = new pushInstruction(
    this.codeStack,
    pushInstructionDoCount
  );
  this["CODE.DO*TIMES"] = new pushInstruction(
    this.codeStack,
    pushInstructionDoTimes
  );
  ...
  this["CODE.LENGTH"] = new pushInstruction(
    this.codeStack,
    pushInstructionLength
  );
  this["CODE.NOOP"] = new pushInstruction(this.codeStack, pushInstructionNoop);

  this["EXEC.DO*RANGE"] = new pushInstruction(
    this.execStack,
    pushInstructionDoRange
  );
  this["EXEC.DO*TIMES"] = new pushInstruction(
    this.execStack,
    pushInstructionDoTimes
  );
  this["EXEC.DO*COUNT"] = new pushInstruction(
    this.execStack,
    pushInstructionDoCount
  );
  ...
  this["BOOLEAN.RAND"] = new pushInstruction(
    this.boolStack,
    pushInstructionRandomBool
  );
  this["INTEGER.RAND"] = new pushInstruction(
    this.intStack,
    pushInstructionRandomNumber
  );
  this["FLOAT.RAND"] = new pushInstruction(
    this.floatStack,
    pushInstructionRandomNumber
  );
  this["NAME.RAND"] = new pushInstruction(
    this.nameStack,
    pushInstructionRandomName
  );
...
function pushRunProgram(inInterpreter, inProgramArray) {
  ...
  while (inInterpreter.execStack.length > 0) {
    ...
    if (inInterpreter._effort > 1000) {
      inInterpreter._errorMessage =
        "Hardcoded effort limit reached (1000 instructions)";
  ...
  for (var i = 0; i < tokens.length; i++) {
    if (!spaces.test(tokens[i])) {
      if (tokens[i] == "(") {
        ...
        if (currentList != null) currentList.push(newList);
        ...
        if (decimal.test(tokens[i])) currentList.push(new pushFloat(num));
        else currentList.push(new pushInt(num));
      } else if (tokens[i] != "") {
        // Instruction token
        if (currentList == null) {
          // alert( 'Push parse error near token "' + tokens[ i ] + '"' );
